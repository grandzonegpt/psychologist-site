...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.MARKETING_IDS = window.MARKETING_IDS || {
    ...
    googleAdsConversionLabel: 'HPtzCM6Tpr8cEMWDvr9D',
  ...
  // Highlight the nav section the current page belongs to
  (function () {
    const p = location.pathname;
    let label = null;
    if (/^\/(blog|tests|practices|library|glossary)/.test(p)) label = /Материалы|Materiały/;
    else if (/^\/topics/.test(p)) label = /Направления|Specjalizacje/;
    if (!label) return;
    document.querySelectorAll('.nav-dropdown summary').forEach((s) => {
      if (label.test(s.textContent)) s.classList.add('nav-current');
    });
  })();
  ...
  (function(){
    ...
    var url = isPl ? '/intro-pl.html' : '/intro.html';
